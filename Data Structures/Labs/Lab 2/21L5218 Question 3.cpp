...
public:
	...
	friend void operator -(char u, myVector& newVector)
	{
...
public:
	...
	friend void operator +(char var, myVector& newVector)
	{

		if (newVector.current_size + 1 > newVector.capacity)
		{
			char* temp = new char[newVector.current_size];
			...
			newVector.arrayptr = new char[newVector.capacity];
			for (int i = 0; i < newVector.current_size; i++)
			{
				newVector.arrayptr[i] = temp[i];
			}
	...
	char* operator+ (const myVector& newVector)
	{
		int i = 0;
		char* newarr = new char[current_size + newVector.current_size];
		for (; i < current_size; i++)
		{
			newarr[i] = arrayptr[i];
...
int main()
{
	...
	cout << vector4;
	char* mergedarr = vector2 + vector4;
	cout << "merged arrray: ";
